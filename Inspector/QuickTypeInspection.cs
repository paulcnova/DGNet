...
public class QuickTypeInspection
{
	#region Properties
	...
	{
		List<string> generics = InspectorUtility.GetGenericParametersString(type.GenericParameters);
		
		this.FillNames(type.FullName.Replace("&", ""), InspectorUtility.GetNamespace(type), generics);
		this.GenericParameters = GenericParameterInspection.CreateArray(type.GenericParameters);
		if(this.GenericParameters.Count == 0)
		{
			this.GenericParameters = InspectorUtility.GetGenericParameters(type.FullName);
		}
	...
	private void FillNames(string typeFullName, string typeNamespace, List<string> generics)
	{
		...
		this.FullName = InspectionRegex.GenericNotation()
			.Replace(InspectorUtility.LocalizeName(typeFullName, generics), "");
		this.Name = InspectorUtility.RemoveNamespaceFromType(InspectorUtility.MakeNameFriendly(this.FullName));
